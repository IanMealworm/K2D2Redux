...
namespace K2D2.Controller
{
    public class DroneSettings
    {
        ...
        public ClampSetting<float> altitude_ratio = new("speed.altitude_ratio", 1, 1,5);
    ...
    public class DronePilot : Pilot
    {
        public static DronePilot Instance { get; set; }

        DroneSettings settings = new DroneSettings();

        public ILogger logger = ReduxLib.ReduxLib.GetLogger("K2D2.DroneController");
        ...
        void compute_Throttle()
        {
            ...
            if (vspeed_mode == VSpeedMode.Direct)
                real_wanted_speed = settings.wanted_speed.V;
            else
            {
                altitude = (float)current_vessel.GetApproxAltitude();
                delta_altitude = settings.wanted_altitude.V - altitude;
                real_wanted_speed = delta_altitude / settings.altitude_ratio.V;
            }
        ...
        void KillHSpeed()
        {
            ...
            var inc = settings.kill_h_speed_ratio.V * H_Speed;
        ...
        void changeSASMode(SASMode new_mode)
        {
            ...
            {
                SASTool.setAutoPilot(AutopilotMode.StabilityAssist);
                settings.kill_h_speed_ratio.V = 0;
            }
        }
        ...
        void changeVSpeedMode(VSpeedMode new_mode)
        {
            ...
            switch(new_mode)
            {
                case VSpeedMode.Altitude:
                    settings.wanted_altitude.V = (float)current_vessel.GetApproxAltitude();
                    break;
                case VSpeedMode.Direct:
                    settings.wanted_speed.V = 0;
                    break;
            }
        }
